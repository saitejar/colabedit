...
from ..model.constants import APPLICATION_NAME, CLIENT_ID, SERVICE

vendor.add("lib")
signin_api_blueprint = Blueprint('signin', __name__)


redis_manager.set('name', 'sai')
print redis_manager.get('name')
...
@signin_api_blueprint.route('/signin_button.png', methods=['GET'])
def signin_button():
    """Returns the button image for sign-in."""
    return send_file('../templates/signin_button.png', mimetype='image/gif')


@signin_api_blueprint.route('/connect', methods=['POST'])
def connect():
    ...
    try:
        ...
        oauth_flow = flow_from_clientsecrets('app/static/data/client_secrets.json', scope='')
...
@signin_api_blueprint.route('/disconnect', methods=['POST'])
def disconnect():
...
@signin_api_blueprint.route('/people', methods=['GET'])
def people():
